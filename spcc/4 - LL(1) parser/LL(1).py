...
cfg = dict()  # create dictionary to store CFG
global non_terminal
...
def find_first(key):
    ...
    for item in value:  # consider individual production rule
        if item[0] in ep:  # if that variable produces epsilon
            epsilon(item)
...
for line in fp:
    line.strip()  # remove beginning and trailing white spaces
    if re.search('\n', line):
        line = line[:line.find('\n')]  # remove \n from line
    ...
    i = 0
    for item in split:
        split[i] = item.strip()  # remove beginning and trailing white spaces
        i = i + 1
    cfg[line[0]] = split  # store LHS as key and RHS as values

print("\nGiven Context Free Grammar is =")
for key, value in cfg.items():
    print(key, " ->", value)  # print CFG
    if '#' in value:  # if any variable/non-terminal generates an epsilon
        ep.append(key)  # then stored that variable in list ep
temp = list()
first = dict()
for key, value in cfg.items():
    ...
    if key in ep:
        # print(non_terminal,"-->#")
        if '#' not in temp:  # if variable produces epsilon
